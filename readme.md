...
├── lib/
│   ├── cryptogpg.sh
│   ├── install.sh      # Contient les fonctions d'installation des prérequis et set_secure_permissions
│   ├── setup.sh        # Gère la création/ouverture/fermeture du conteneur
│   └── ssh_config_manager.sh
└── config/
    └── alias.sh        # Créé par ssh_setup, contient l'alias 'evsh'
└── secure_env_data/
    └── secure_container.img # Le fichier de l'environnement chiffré (sera créé ici)
